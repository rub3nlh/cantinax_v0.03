-- Fix custom package deliveries to properly distribute meals across days
-- This migration updates the create_order_deliveries function to handle custom packages correctly
-- Version without the view that was causing errors

-- Drop any existing triggers or functions that might be enforcing the constraint
DROP TRIGGER IF EXISTS create_deliveries_on_order ON orders;
DROP FUNCTION IF EXISTS create_order_deliveries();

-- Check for any triggers on the delivery_meals table
DO $$
DECLARE
...
-- Drop all triggers on the delivery_meals table
DO $$
DECLARE
  trigger_record record;
BEGIN
  FOR trigger_record IN
    ...
    WHERE tgrelid = 'delivery_meals'::regclass
  LOOP
    EXECUTE 'DROP TRIGGER IF EXISTS ' || trigger_record.tgname || ' ON delivery_meals';
  END LOOP;
END $$;
