-- Fix create_order_deliveries function to handle meal counts correctly
DROP TRIGGER IF EXISTS create_deliveries_on_order ON orders;
DROP FUNCTION IF EXISTS create_order_deliveries();

-- Create enhanced function with fixed meal count handling
...
INSERT INTO trigger_logs (
  ...
  'migration',
  'fix_meal_count_in_deliveries',
  'system',
  jsonb_build_object(
    'description', 'Fixed create_order_deliveries function to handle meal counts correctly',
    'version', '1.0.0'
  ),
  true
);
