-- Update create_order_deliveries function to use daily deliveries with one meal per day
DROP TRIGGER IF EXISTS create_deliveries_on_order ON orders;
DROP FUNCTION IF EXISTS create_order_deliveries();

-- Create enhanced function with daily deliveries and one meal per day
...
BEGIN
  ...
  WHILE remaining_meals > 0 LOOP
    -- Create delivery record - Changed to limit 1 meal per delivery
    INSERT INTO order_deliveries (
      ...
      delivery_date,
      'pending',
      LEAST(1, remaining_meals)  -- Changed from 2 to 1 meal per delivery
    ...
    INSERT INTO trigger_logs (
      ...
      jsonb_build_object(
        ...
        'meals_to_add', LEAST(1, remaining_meals),  -- Changed from 2 to 1
        'remaining_total', remaining_meals
      ),
      true
    );
    
    -- Add 1 meal to this delivery, respecting counts (changed from up to 2 meals)
    i := 0;
    j := 0;
    
    -- Keep adding meals until we've added 1 or run out (changed from 2 to 1)
    WHILE j < LEAST(1, remaining_meals) AND i < jsonb_array_length(meals_array) LOOP
      meal_data := meals_array->i;
      ...
      -- If we've added 1 meal or run out of meals, stop (changed from 2 to 1)
      IF j >= 1 OR remaining_meals <= 0 THEN
        EXIT;
      END IF;
    END LOOP;
...
INSERT INTO trigger_logs (
  ...
  'migration',
  'fix_daily_deliveries_one_meal_per_day',
  'system',
  jsonb_build_object(
    'description', 'Updated create_order_deliveries function to use daily deliveries with one meal per day',
    'version', '1.0.0'
  ),
  true
);
