-- Fix UUID error in create_order_deliveries function exception handling
DROP TRIGGER IF EXISTS create_deliveries_on_order ON orders;
DROP FUNCTION IF EXISTS create_order_deliveries();

-- Create enhanced function with fixed exception handling
...
EXCEPTION WHEN OTHERS THEN
  -- Log any errors - FIX: Use a valid UUID as fallback instead of 'unknown'
  INSERT INTO trigger_logs (
    trigger_name,
    function_name,
    record_id,
...
INSERT INTO trigger_logs (
  ...
  'migration',
  'fix_uuid_error_in_custom_package',
  'system',
  jsonb_build_object(
    'description', 'Fixed UUID error in create_order_deliveries function exception handling',
    'version', '1.0.0'
  ),
  true
);
