-- Remove the 2-meal limit per delivery for custom packages
DROP TRIGGER IF EXISTS create_deliveries_on_order ON orders;
DROP FUNCTION IF EXISTS create_order_deliveries();

-- Create enhanced function without meal limit per delivery for custom packages
...
BEGIN
  ...
  WHILE remaining_meals > 0 LOOP
    ...
    -- Create delivery record - REMOVED LIMIT FOR CUSTOM PACKAGES
    INSERT INTO order_deliveries (
      ...
      delivery_date,
      'pending',
      meals_for_current_day  -- No limit on meals_count
    ...
    -- REMOVED LIMIT FOR CUSTOM PACKAGES
...
EXCEPTION WHEN OTHERS THEN
  -- Log any errors - Use a valid UUID as fallback
  INSERT INTO trigger_logs (
    trigger_name,
    function_name,
    record_id,
    ...
    'create_deliveries_on_order',
    'create_order_deliveries',
    COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid),  -- Use nil UUID as fallback
    jsonb_build_object(
      'error', SQLERRM,
...
-- Check if there's a constraint on delivery_meals table limiting meals per delivery
DO $$
DECLARE
  constraint_exists boolean;
BEGIN
  -- Check if there's a constraint named 'delivery_meals_max_meals_per_delivery'
  SELECT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'delivery_meals_max_meals_per_delivery'
  ) INTO constraint_exists;
  
  -- If the constraint exists, drop it
  IF constraint_exists THEN
    EXECUTE 'ALTER TABLE delivery_meals DROP CONSTRAINT delivery_meals_max_meals_per_delivery';
  END IF;
END $$;
...
INSERT INTO trigger_logs (
  ...
  'migration',
  'remove_meal_limit_per_delivery',
  'system',
  jsonb_build_object(
    'description', 'Removed the 2-meal limit per delivery for custom packages',
    'version', '1.0.0'
  ),
  true
);
