/*
  # Update orders and deliveries schema for meal grouping

  1. Changes
    - Add delivery_groups array to orders table
    - Add delivery_date to each meal group
    - Add status tracking for each meal group
    - Update triggers to handle meal groups
...
BEGIN
  ...
  FOR i IN 0..delivery_groups_count-1 LOOP
    delivery_status := (NEW.delivery_groups->i->>'status')::text;
  ...
  all_completed := delivery_groups_count = completed_count;
  ...
  ELSIF EXISTS (
    ...
    WHERE (d->>'status')::text = 'in_progress'
...
BEGIN
  ...
  SELECT m INTO meal_to_complete
  FROM jsonb_array_elements(current_group->'meals') AS m
  WHERE m->>'id' = meal_id;
  ...
  IF jsonb_array_length(completed_meals) = jsonb_array_length(current_group->'meals') THEN
    current_group := jsonb_set(current_group, '{status}', '"ready"');
  ELSE
    current_group := jsonb_set(current_group, '{status}', '"in_progress"');
  END IF;
