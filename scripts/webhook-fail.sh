...
# Ask for failure state
read -p "Enter the failure state (2=rejected, 3=expired, 4=cancelled, or press Enter for default 2): " STATE_INPUT
if [ -n "$STATE_INPUT" ]; then
  STATE=$STATE_INPUT
fi

# Get failure reason based on state
get_failure_reason() {
  case $1 in
    2) echo "Payment was rejected by the payment processor" ;;
    3) echo "Payment link has expired" ;;
    4) echo "Payment was cancelled by the user" ;;
    *) echo "Unknown failure reason" ;;
  esac
}

FAILURE_REASON=$(get_failure_reason $STATE)
...
echo -e "\nSuccess! The payment has been marked as failed with state $STATE."
